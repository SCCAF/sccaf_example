...
def run_ACTINN(ad, ad_ref):
    ad1 = ad_ref[:,ad_ref.var_names.isin(ad.var_names)]

    type_to_label_d = type_to_label_dict(ad1.obs['cell'])
    label_to_type_dict = {v: k for k, v in type_to_label_d.items()}
    nt = len(set(ad1.obs['cell']))
    train_label = convert_type_to_label(ad1.obs['cell'].tolist(), type_to_label_d)
    train_label = one_hot_matrix(train_label, nt)
    train_set = pd.DataFrame(ad1.X.todense(),index=ad1.obs_names, columns= ad1.var_names).T
    test_set = pd.DataFrame(ad[:,ad1.var_names].X.todense(),index=ad[:,ad1.var_names].obs_names, columns= ad[:,ad1.var_names].var_names).T
    train_set, test_set = scale_sets([train_set, test_set])
    parameters = model(train_set, train_label, test_set, 0.0001, 50, 128, True)
    test_predict = predict(test_set, parameters)
    return(pd.Series(test_predict).map(label_to_type_dict).tolist())

import time
t0 = time.time()
x = run_ACTINN(ad, ad_ref)
t1 = time.time() - t0
print(t1)
pd.Series(x).to_csv("%s_%s_ACTINN.csv"%(sys.argv[1],sys.argv[2]))